...
**Solution.** $\theta(t) = \theta_m\cos(2\pi t/T_0 + \varphi)$ ; $T_0 = 2\pi\sqrt{J_\Delta/C}$. Graphique : $\boxed{T_0 = 4{,}0\text{ s}}$, $\theta_m = T_0\dot\theta_m/2\pi = \boxed{0{,}17\text{ rad}}$. D'où $\boxed{C = 4\pi^2J_\Delta/T_0^2 = 7{,}97\times10^{-2}\text{ N·m/rad}}$.
**Énergie.** $E_m$ conservée : $E_m = \tfrac12J_\Delta\dot\theta_m^2 = \boxed{8{,}07\times10^{-4}\text{ J}}$. Travail du couple : $W_C = \Delta E_c = \tfrac{J_\Delta}{2}[\dot\theta^2(t_2) - \dot\theta^2(t_1)] = \boxed{7{,}99\times10^{-4}\text{ J}}$ ($\dot\theta$ : 0 → 0,2 rad/s).
